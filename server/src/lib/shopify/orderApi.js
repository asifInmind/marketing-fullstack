...
import { parseCustomerJourneySummary } from './customerJourneySummaryParser.js';
...
export async function syncOrdersFromShopify(shopDomain, shopify_token, startDate, endDate) {
  ...
  const orderPromises = allOrders.map(order => {
    ...
    const { landingSite, referringSite, attribution, customerJourney } = parseCustomerJourneySummary(order);
